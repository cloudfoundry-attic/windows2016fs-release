...
Push-Location $ROOTDIR
  $image_tag=(cat src/code.cloudfoundry.org/windows2016fs/IMAGE_TAG)
  $image_name="cloudfoundry/windows2016fs"
  $output_dir="blobs/windows2016fs"
  mkdir -Force $output_dir

  go run src/code.cloudfoundry.org/windows2016fs/cmd/hydrate/main.go -image $image_name -outputDir $output_dir -tag $image_tag
